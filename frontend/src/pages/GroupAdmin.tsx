...
const GroupAdmin = () => {
  ...
  const [syllabusData, setSyllabusData] = useState<{ topics: topics[] }>({
    topics: [],
  });
  ...
  const [showDeleteResourceConfirmation, setShowDeleteResourceConfirmation] =
    useState(false);
    ...
    const fetchPendingResources = async () => {
      ...
      if (groupCode) {
        ...
        try {
          ...
          if (data.success) {
            const pendingResourcesData =
              data.message?.pendingResources || data.pendingResources || [];
    ...
    const fetchReportedResources = async () => {
      ...
      if (groupCode && activeTab === "reported") {
        ...
        try {
          ...
          if (data.success) {
            const reportedResourcesData =
              data.message?.reportedResources || data.reportedResources || [];
  ...
  const handleRejectResource = async (resourceId: number, fileName: string) => {
    ...
    try {
      const { data } = await rejectResourceAPI(
        groupCode,
        resourceId,
        reason || undefined
      );
  ...
  const handleConfirmRemoveMember = async () => {
    ...
    try {
      ...
      if (result.success) {
        setMembers((prev) =>
          prev.filter((member) => member.userId !== memberToRemove.id)
        );
  ...
  const handleConfirmDeleteResource = async () => {
    ...
    try {
      const { data } = await deleteApprovedResourceAPI(
        group.groupCode,
        resourceToDelete.id
      );
      if (data.success) {
        ...
        setGroup((prev) => {
          ...
          return {
            ...prev,
            AdditionalResources:
              prev.AdditionalResources?.filter(
    ...
    if (group?.syllabus?.topics) {
      ...
      const topicsWithSortedSubtopics = sortedTopics.map((topic) => ({
        ...
        subTopics: topic.subTopics
          ? [...topic.subTopics].sort((a: any, b: any) => {
              const dateA = new Date(a.created_at || 0);
              const dateB = new Date(b.created_at || 0);
              return dateA.getTime() - dateB.getTime();
            })
  ...
  const filteredResources =
    group?.AdditionalResources?.filter((resource) => {
      const resourceName = resource.filePath?.split("/").pop() || "";
      return (
        resourceName.toLowerCase().includes(resourceSearchTerm.toLowerCase()) ||
        resource.fileType
          ?.toLowerCase()
    ...
    >
          ...
          <div className="layout-content-container flex flex-col w-full max-w-7xl mx-auto flex-1 px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pb-4 sm:pb-6">
              <div className="flex flex-col gap-1">
                <p className="text-slate-900 text-xl sm:text-2xl font-bold leading-tight">
                  Study Group Admin Panel
                </p>
                <p className="text-slate-500 text-sm font-normal leading-normal">
                  Manage group settings, reports, and members.
                </p>
              </div>
            </div>
            ...
            <div className="space-y-6 sm:space-y-8">
              ...
              <section>
                ...
                <div className="space-y-4 sm:space-y-6 max-w-full lg:max-w-xl">
                  <div>
                    <label
                      className="block text-slate-700 text-sm font-medium leading-normal pb-1.5"
                      htmlFor="groupName"
                    >
                      Group Name
                    </label>
                    ...
                    <div className="flex justify-start gap-3">
                      ...
                      >
                        {isSavingGroupDetails ? (
                          <>
                            <span className="material-icons-outlined text-lg mr-2 animate-spin">
                              sync
                            </span>
                  ...
                  {!isCreator && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                      <div className="flex items-start">
                        <span className="material-icons-outlined text-blue-600 mr-2 mt-0.5">
                          info
                        </span>
                ...
                {group?.syllabus?.topics && group.syllabus.topics.length > 0 ? (
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-slate-200">
                        <thead className="bg-slate-50">
                          <tr>
                            <th
                              className="px-3 sm:px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-slate-500"
                              scope="col"
                            >
                              Topic
                            </th>
                  ...
                  <div className="rounded-lg border border-slate-200 bg-white shadow-sm p-6 text-center">
                    ...
                    <p className="text-slate-500 text-sm">
                      No syllabus available for this group.
                    </p>
                    {isCreator && (
                      <button
                            ...
                            <tr key={member.id}>
                              <td className="px-3 sm:px-4 py-3 text-sm font-medium text-slate-800">
                                <div className="truncate max-w-xs">
                                  {member.UserModel?.fullName ||
                                    member.UserModel?.username ||
                                    "Unknown"}
                                </div>
                              </td>
              ...
              {/* Resources Section */}
              <section>
                <h3 className="text-slate-800 text-xl font-semibold leading-tight tracking-tight mb-4">
                  Resource Management
                </h3>
                ...
                <div className="mb-4">
                  <div className="border-b border-slate-200">
                    <nav className="-mb-px flex space-x-4 sm:space-x-8 overflow-x-auto">
                      ...
                      >
                        Approved Resources (
                        {group?.AdditionalResources?.length || 0})
                      </button>
                      <button
                        className={`py-2 px-1 border-b-2 font-medium text-sm whitespace-nowrap ${
                          activeTab === "pending"
                ...
                <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow">
                  <div className="overflow-x-auto">
                    <table className="w-full min-w-full">
                      <thead className="bg-slate-50">
                        <tr>
                          <th className="px-3 sm:px-4 py-3 sm:py-3.5 text-left text-slate-600 text-xs sm:text-sm font-semibold">
                            Resource Name
                          </th>
                          <th className="px-3 sm:px-4 py-3 sm:py-3.5 text-left text-slate-600 text-xs sm:text-sm font-semibold">
                            Type
                          </th>
                          <th className="px-3 sm:px-4 py-3 sm:py-3.5 text-left text-slate-600 text-xs sm:text-sm font-semibold hidden sm:table-cell">
                            Uploaded By
                          </th>
                          <th className="px-3 sm:px-4 py-3 sm:py-3.5 text-left text-slate-600 text-xs sm:text-sm font-semibold hidden md:table-cell">
                            Uploaded Date
                          </th>
                          <th className="px-3 sm:px-4 py-3 sm:py-3.5 text-left text-slate-600 text-xs sm:text-sm font-semibold">
                            Status
                      ...
                      <tbody className="divide-y divide-slate-200">
                        {activeTab === "approved" ? (
                          ...
                          loadingPending ? (
                            ...
                            <tr>
                              ...
                              >
                                {resourceSearchTerm
                                  ? "No pending resources found matching your search."
                              ...
                              .filter((resource) => {
                                ...
                                return (
                                  resourceName
                                    .toLowerCase()
                                    .includes(
                                      resourceSearchTerm.toLowerCase()
                                    ) ||
                                  resource.fileType
                                    ?.toLowerCase()
                                    .includes(resourceSearchTerm.toLowerCase())
                        ...
                        loadingReported ? (
                          ...
                          <tr>
                            ...
                            >
                              {resourceSearchTerm
                                ? "No reported resources found matching your search."
                            ...
                            .map((resource, index) => (
                              <tr key={resource.id || index}>
                                ...
                                <td className="px-3 sm:px-4 py-3">
                                  <span
                                    className={`inline-flex items-center px-2 sm:px-2.5 py-0.5 rounded-full text-xs font-medium ${
                                      resource.report.status === "pending"
                                        ? "bg-red-100 text-red-800"
                                        : resource.report.status === "reviewed"
                                        ? "bg-yellow-100 text-yellow-800"
                                        : resource.report.status === "resolved"
                                        ? "bg-green-100 text-green-800"
                                  ...
                                  >
                                    {resource.report.status === "pending"
                                      ...
                                      : resource.report.status === "resolved"
                                      ? "Resolved"
                                      : resource.report.status === "dismissed"
                                      ? "Dismissed"
                                      ...
                                      onClick={() => {
                                        alert(
                                          `Report Details:\nReason: ${
                                            ...
                                            resource.report.reporter.username
                                          }\nReported on: ${new Date(
                                            resource.report.createdAt
                                          ).toLocaleDateString()}`
                                        );
                                      }}
                                    >
              ...
              <section>
                <h2 className="text-slate-800 text-xl font-semibold leading-tight tracking-tight pb-4">
                  Group Details
                </h2>
                <div className="rounded-lg border border-slate-200 bg-white shadow-sm divide-y divide-slate-200">
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 p-3 sm:p-4 items-center">
                    ...
                    <p className="text-slate-800 text-sm font-medium sm:col-span-2">
                      {members.length}
                    </p>
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 p-3 sm:p-4 items-center">
                    ...
                    <div className="text-slate-800 text-sm font-medium sm:col-span-2 break-all flex items-center justify-between">
                      <span>{group.groupCode}</span>
                      <button
                        onClick={handleCopyGroupCode}
                        className="ml-2 p-1 rounded hover:bg-gray-100 transition-colors duration-200 flex-shrink-0"
                        title={isCopied ? "Copied!" : "Copy group code"}
                      >
                        <FontAwesomeIcon
      ...
      <ConfirmationModal
        ...
        message={`Are you sure you want to delete "${
          resourceToDelete?.name || "this resource"
        ...
        <DialogContent className="max-w-5xl max-h-[95vh] overflow-hidden flex flex-col">
          <DialogHeader className="border-b border-slate-200 pb-4">
            ...
            <DialogDescription className="text-slate-600">
              Modify the syllabus structure for your group. Add, remove, or edit
              topics and subtopics to keep your curriculum organized.
            </DialogDescription>
          </DialogHeader>
              ...
              {syllabusData.topics.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-slate-400 text-sm mb-6">
                    Start building your syllabus by adding your first topic
                  </p>
                  <Button
                    variant="outline"
                    onClick={addTopic}
                ...
                syllabusData.topics.map((topic, topicIndex) => (
                  <div
                    key={topicIndex}
                    className="border border-slate-200 rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow duration-200"
                  >
                      ...
                      {topic.subTopics && topic.subTopics.length > 0 ? (
                        <div className="space-y-3">
                          {topic.subTopics.map((subtopic, subTopicIndex) => (
                            <div
                              key={subTopicIndex}
                              className="flex items-start gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200 hover:bg-slate-100 transition-colors duration-150"
                            >
                        ...
                        <div className="text-center py-8 border-2 border-dashed border-slate-200 rounded-lg bg-slate-50">
                          <span className="material-icons-outlined text-slate-400 text-4xl block mb-2">
                            list_alt
                          </span>
          ...
          <DialogFooter className="border-t border-slate-200 pt-4 flex flex-col sm:flex-row gap-3">
            ...
            >
              {isSavingSyllabus ? (
                <>
                  <span className="material-icons-outlined text-sm mr-2 animate-spin">
                    sync
                  </span>
                  Saving...
                </>

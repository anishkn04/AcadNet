...
const ReportUserModal: React.FC<ReportUserModalProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await reportUserAPI(groupCode, reportedUserId, reportData);
      
      if (response.data.success) {
        toast.success('User reported successfully. Group admins will review the report.');
        handleClose();
      } else {
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          ...
          <DialogDescription>
            You are reporting <strong>{reportedUserName}</strong>. Please provide a reason for this report.
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-md p-3">
            <p className="text-sm text-yellow-800">
              ...
              Please ensure your report is legitimate and based on actual violations.
            </p>
          </div>
